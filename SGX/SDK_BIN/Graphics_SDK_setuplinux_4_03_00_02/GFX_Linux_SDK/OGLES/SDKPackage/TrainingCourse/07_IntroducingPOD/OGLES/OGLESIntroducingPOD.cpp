...
bool OGLESIntroducingPOD::RenderScene()
{
	...
	*/
	...
	m_iTimePrev	= iTime;
	m_fFrame	+= VERTTYPEMUL(f2vt(iDeltaTime), f2vt(g_fDemoFrameRate));
	...
	*/
	for(int i = 0; i < (int)m_Scene.nNumMeshNode; ++i)
	{
		...
		else
		{
			...
			myglMaterialv(GL_FRONT_AND_BACK, GL_AMBIENT,  PVRTVec4(Material.pfMatAmbient,  f2vt(1.0f)).ptr());
			myglMaterialv(GL_FRONT_AND_BACK, GL_DIFFUSE,  PVRTVec4(Material.pfMatDiffuse,  f2vt(1.0f)).ptr());
		}
