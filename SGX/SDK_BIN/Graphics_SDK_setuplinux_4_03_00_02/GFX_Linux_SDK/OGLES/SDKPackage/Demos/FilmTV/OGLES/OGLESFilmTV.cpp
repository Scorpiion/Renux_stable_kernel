...
/******************************************************************************
 Defines
******************************************************************************/
// The iPhone doesn't have EGL so has no PBuffers so on this platform we ifdef out the unsupported features
...
const VERTTYPE g_fCameraNear = f2vt(1.0f);
const VERTTYPE g_fCameraFar  = f2vt(150.0f);
...
const char c_szSceneFile[]			= "FilmTVScene_fixed.pod";
#else
const char c_szSceneFile[]			= "FilmTVScene_float.pod";
#endif
...
bool OGLESFilmTV::InitView()
{
	...
	PVRTVec3	vFrom, vTo(f2vt(0.0f)), vUp(f2vt(0.0f), f2vt(1.0f), f2vt(0.0f));
	VERTTYPE fFOV;
	...
	m_Projection = PVRTMat4::PerspectiveFovRH(fFOV, f2vt((float)PVRShellGet(prefWidth)/(float)PVRShellGet(prefHeight)), g_fCameraNear, g_fCameraFar, PVRTMat4::OGL, bRotate);
...
void OGLESFilmTV::CalcMiniCameraView()
{
	...
	m_MiniCamView  = PVRTMat4::RotationX(f2vt(((float)atan2(fX, 10.0f))));
	m_MiniCamView *= PVRTMat4::RotationY(f2vt(fCamRot));
	m_MiniCamView *= PVRTMat4::RotationZ(f2vt(((float)atan2(fZ, 10.0f))));

	// Setup the mini camera's view projection matrix
	m_MiniCamProj = PVRTMat4::PerspectiveFovRH(f2vt(70.0f * (PVRT_PIf / 180.0f)), f2vt(1), g_fCameraNear, g_fCameraFar, PVRTMat4::OGL, false);
}
...
bool OGLESFilmTV::RenderScene()
{
	...
	if(StartRenderToTexture())
	{
		...
		myglLoadMatrix(m_MiniCamProj.f);
