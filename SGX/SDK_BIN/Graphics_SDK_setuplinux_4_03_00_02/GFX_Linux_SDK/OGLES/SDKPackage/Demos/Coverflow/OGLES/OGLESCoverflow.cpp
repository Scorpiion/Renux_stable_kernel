...
bool OGLESCoverflow::InitView()
{
	...
	PVRTVECTOR3	vFrom = {f2vt(0.0f), f2vt(0.0f), f2vt(15.0f) },
						vTo = { f2vt(0), f2vt(0), f2vt(0) },
						vUp = { f2vt(0), f2vt(1), f2vt(0) };

	PVRTMatrixPerspectiveFovRH(m_mProjection, f2vt(g_FOV), f2vt((float)PVRShellGet(prefWidth)/(float)PVRShellGet(prefHeight)), f2vt(g_fCameraNear), f2vt(g_fCameraFar), bRotate);
...
void OGLESCoverflow::CreateCover()
{
	...
	PVRTVec3 normal = PVRTVec3(f2vt(0.0f), f2vt(1.0f), f2vt(0.0f)); //all the normals are the same
	...
	*/
	...
	m_coverPoints[0].p	=	PVRTVec3(f2vt(-dim), f2vt(dim), 0);
	m_coverPoints[1].p	=	PVRTVec3(f2vt(-dimLess), f2vt(dim), 0);
	m_coverPoints[2].p	=	PVRTVec3(f2vt(dimLess), f2vt(dim), 0);
	m_coverPoints[3].p	=	PVRTVec3(f2vt(dim), f2vt(dim), 0);
	m_coverPoints[4].p	=	PVRTVec3(f2vt(-dim), f2vt(dimLess), 0);
	m_coverPoints[5].p	=	PVRTVec3(f2vt(-dimLess), f2vt(dimLess), 0);
	m_coverPoints[6].p	=	PVRTVec3(f2vt(dimLess), f2vt(dimLess), 0);
	m_coverPoints[7].p	=	PVRTVec3(f2vt(dim), f2vt(-dimLess), 0);
	m_coverPoints[8].p	=	PVRTVec3(f2vt(-dim), f2vt(-dimLess), 0);
	m_coverPoints[9].p	=	PVRTVec3(f2vt(-dimLess), f2vt(-dimLess), 0);
	m_coverPoints[10].p	=	PVRTVec3(f2vt(dimLess), f2vt(-dimLess), 0);
	m_coverPoints[11].p	=	PVRTVec3(f2vt(dim), f2vt(-dimLess), 0);
	m_coverPoints[12].p	=	PVRTVec3(f2vt(-dim), f2vt(-dim), 0);
	m_coverPoints[13].p	=	PVRTVec3(f2vt(-dimLess), f2vt(-dim), 0);
	m_coverPoints[14].p	=	PVRTVec3(f2vt(dimLess), f2vt(-dim), 0);
	m_coverPoints[15].p	=	PVRTVec3(f2vt(dim), f2vt(-dim), 0);


	for(i = 0; i < 16; ++i)
	{
		m_coverPoints[i].n = normal;
		m_coverPoints[i].c = PVRTVec4(f2vt(1.0f), f2vt(1.0f), f2vt(1.0f), f2vt(0.0f));

		//the uvs are matched to the positions (+0.5 for range 0-1)
		m_coverPoints[i].t.x = m_coverPoints[i].p.x + f2vt(0.5f);
		m_coverPoints[i].t.y = m_coverPoints[i].p.y + f2vt(0.5f);

		//scale up to desired size
		m_coverPoints[i].p.x = VERTTYPEMUL(m_coverPoints[i].p.x, f2vt(width));
		m_coverPoints[i].p.y = VERTTYPEMUL(m_coverPoints[i].p.y, f2vt(height));
	}

	//only the center 4 vertices have are fully opaque all the rest around the edge are tranparent
	m_coverPoints[5].c.w = f2vt(1.0f);
	m_coverPoints[6].c.w = f2vt(1.0f);
	m_coverPoints[9].c.w = f2vt(1.0f);
	m_coverPoints[10].c.w = f2vt(1.0f);
	...
	for(row = 0; row < 4; ++row)
	{
		...
		float dark = 1 - ((vt2f(m_coverPoints[(row*4)].p.y)/height)+ 0.5f);
		...
		for(col = 0; col < 4; ++col)
		{
			...
			m_coverPoints[((row*4)+col) + 16].p.y = VERTTYPEMUL(m_coverPoints[((row*4)+col) + 16].p.y, f2vt(-1.0f));
			m_coverPoints[((row*4)+col) + 16].p.y -= f2vt(height + heightFromMirror);

			m_coverPoints[((row*4)+col) + 16].c.x = f2vt(dark);
			m_coverPoints[((row*4)+col) + 16].c.y = f2vt(dark);
			m_coverPoints[((row*4)+col) + 16].c.z = f2vt(dark);
		}
	}
...
void OGLESCoverflow::DrawInPosition(int index, float queueLerp, int coverIndex)
{
	...
	PVRTMatrixRotationY(mRotation, f2vt(angle));

	myglLoadMatrix((mTrans * m_mView * mRotation).f);
...
void OGLESCoverflow::DrawMesh()
{
	...
	glVertexPointer(3, VERTTYPEENUM, stride, 0);
	glNormalPointer(VERTTYPEENUM, stride, (GLvoid*)offsetof(SVertex, n));
	glColorPointer(4, VERTTYPEENUM, stride, (GLvoid*)offsetof(SVertex, c));
	glTexCoordPointer(2, VERTTYPEENUM, stride, (GLvoid*)offsetof(SVertex, t));
